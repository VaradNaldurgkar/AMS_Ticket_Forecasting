...
const AddExcel = () => {
  ...
  const [uploadType, setUploadType] = useState("ams");
  ...
  const handleUpload = async () => {
    ...
    if (uploadType === "ams" && files.length !== 2) {
      alert("AMS requires exactly 2 files (IM + RR)");
      return;
    }

    if (uploadType !== "ams" && files.length !== 1) {
  ...
  return (
    <div className="excel-page">
      ...
      <div className="excel-header">
        ...
        <p>Upload Excel datasets for AMS analysis and forecasting.</p>
      </div>
      ...
      <div className="excel-stats-grid">
        ...
        <div className="excel-stat-card">
          ...
          <div className="excel-stat-info">
            <h4>Last Upload</h4>
            <h2>{uploads.length > 0 ? uploads[0].upload_date : "--"}</h2>
  ...
  <h4>Select Dataset Type</h4>

  <div className="upload-type-options">

    <div
      className={`upload-type-card ${uploadType === "ams" ? "active" : ""}`}
      onClick={() => setUploadType("ams")}
    >
      <FaDatabase size={22} />
      <span>AMS Tickets</span>
      <small>Incident + Service Request</small>
    </div>

    <div
      className={`upload-type-card ${uploadType === "asset" ? "active" : ""}`}
      onClick={() => setUploadType("asset")}
    >
      <FaFolderOpen size={22} />
      <span>IT Asset</span>
      <small>Asset Requisition</small>
    </div>

    <div
      className={`upload-type-card ${uploadType === "software" ? "active" : ""}`}
      onClick={() => setUploadType("software")}
    >
      <FaFileExcel size={22} />
      <span>Software</span>
      <small>Software Requisition</small>
    </div>
          ...
          {files.length > 0 &&
            files.map((file, index) => (
              <div className="excel-file-preview" key={index}>
                <div>
        ...
        <div className="excel-guidelines-card">
          ...
          <div className="excel-info-box">
            Make sure your data follows AMS standards for accurate forecasting and analysis.
          </div>
        </div>
      </div>
      ...
      {showSuccess && (
        <div className="excel-modal-overlay">
          <div className="excel-success-modal">
            <div className="excel-success-icon">✓</div>
